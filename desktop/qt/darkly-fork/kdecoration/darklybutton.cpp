...
void Button::drawIcon(QPainter *painter) const
{
    ...
    */
    ...
    // ---- Glass: close / maximise / minimise are small round dots that are always there (design/tokens.json
    // decoration.buttonSize); the glyph appears on hover. Close is the brightest dot and turns red under the pointer.
    if (type() == DecorationButtonType::Close || type() == DecorationButtonType::Maximize || type() == DecorationButtonType::Minimize) {
        const bool active = c && c->isActive();
        const bool hot = isHovered() || isPressed();
        const qreal dot = qMin<qreal>(18.0, (d ? d->internalSettings()->glassDotSize() : 16) * 20.0 / qMax<qreal>(1.0, width));   // GlassDotSize px on screen, in the 20-unit space
        const QRectF dotRect((18 - dot) / 2, (18 - dot) / 2, dot, dot);
        QColor ink = d ? d->fontColor() : QColor(Qt::white);
        QColor fill = ink;
        // no glyphs at all: idle dots are monochrome, under the pointer each one takes its own colour
        if (hot) fill = type() == DecorationButtonType::Close ? QColor(0xe5, 0x48, 0x4d) : type() == DecorationButtonType::Minimize ? QColor(0xf6, 0xa0, 0x3a) : QColor(0x4c, 0xc3, 0x8a);
        else fill.setAlphaF(type() == DecorationButtonType::Close ? (active ? 0.36 : 0.14) : (active ? 0.17 : 0.09));
        if (isPressed()) fill = fill.darker(118);
        painter->setPen(Qt::NoPen); painter->setBrush(fill); painter->drawEllipse(dotRect);
        return;
    }
