...
void Helper::renderMenuFrame(QPainter *painter, const QRect &rect, const QColor &color, const QColor &outline, bool roundCorners) const
{
    ...
    if (roundCorners) {
        ...
        // outline — Glass: the same lit hairline as every other glass surface, whatever colour was asked for.
        // Stroked half a pixel INSIDE the frame (the menu's window mask clips anything on or past the edge, which is
        // what ate the corners), and the four arcs are stroked twice: a 1 px anti-aliased arc is much dimmer than a
        // straight run at this alpha.
        if (outline.isValid()) {
            painter->setCompositionMode(QPainter::CompositionMode_SourceOver);
            painter->setBrush(Qt::NoBrush);
            const QRectF o = QRectF(rect).adjusted(1.0, 1.0, -1.0, -1.0);
            const qreal r = qMax<qreal>(1.0, radius - 1.0);
            QLinearGradient lit(0, o.top(), 0, o.bottom());
            lit.setColorAt(0.0, QColor(255, 255, 255, 52));
            lit.setColorAt(1.0, QColor(255, 255, 255, 26));
            painter->setPen(QPen(QBrush(lit), 1.0));
            ...
            // On a composited desktop the Glass effect adds its own 2 px rim along the menu's straight edges, but the menu's
            // rounded window mask clips that rim at the corners. Give the arcs the same 2 px weight so the border reads as
            // one continuous line.
            painter->setPen(QPen(QColor(255, 255, 255, 46), 2.0));
...
void Helper::renderSelection(QPainter *painter, const QRect &rect, const QColor &color, Corners corners) const
{
    ...
    // Glass: a selection is a small pane of glass — translucent fill (from the colour scheme) plus a lit hairline rim
    if (color.alpha() > 0 && rect.height() > 8 && rect.width() > 8) {
        QLinearGradient rim(0, rect.top(), 0, rect.bottom());
        rim.setColorAt(0.0, QColor(255, 255, 255, 58));
        rim.setColorAt(1.0, QColor(255, 255, 255, 22));
        painter->setPen(QPen(QBrush(rim), 1.0));
        painter->setBrush(Qt::NoBrush);
        painter->drawPath(roundedPath(QRectF(rect).adjusted(0.5, 0.5, -0.5, -0.5), corners, StyleConfigData::cornerRadius()));
        painter->setPen(Qt::NoPen);
    }
}
