...
// ---- Glass: frosted sidebars for QML (Kirigami) views embedded in widget windows — System Settings' sidebar is one.
// Darkly's translucency only knows QWidget chrome. A QQuickWidget that is a narrow column of its window gets a
// transparent scene, and every Kirigami theme object inside it gets a fully transparent background colour, so the
// window's own frosted fill (painted by the style, see the Paint branch of eventFilter) shows through. Wide QQuickWidgets
// (the settings pages themselves) are left alone: content stays solid (design variant B).
namespace {
class GlassQuickSidebar : public QObject
{
public:
    explicit GlassQuickSidebar(QQuickWidget *w) : QObject(w), m_w(w)
    {
        w->installEventFilter(this);
        auto *tick = new QTimer(this); tick->setInterval(180);
        QObject::connect(tick, &QTimer::timeout, this, [this] { if (m_w && m_w->isVisible() && m_w->window() && m_w->window()->isActiveWindow()) apply(); });
        tick->start();
        QObject::connect(w, &QQuickWidget::statusChanged, this, [this] { later(); });
        later();
    }
    bool eventFilter(QObject *, QEvent *e) override
    {
        if (e->type() == QEvent::Show || e->type() == QEvent::Resize) later();
        return false;
    }
private:
    void later() { for (int ms : {0, 400, 1500}) QTimer::singleShot(ms, this, [this] { apply(); }); }
    // hairlines are light on dark surfaces and dark on light ones: asked from the live application palette on every pass, so
    // a colour-scheme switch while the app runs is followed
    static bool lightUi() { return qApp->palette().color(QPalette::Window).lightness() > 140; }
    // A delegate's background rectangle gets a hairline rim whose strength follows the fill: nothing when the row is idle
    // (transparent fill), faint on hover, clear when selected. Kept in step by the poller in later()/apply().
    static void refreshRim(QQuickItem *bg)
    {
        if (!bg || !QByteArray(bg->metaObject()->className()).contains("Rectangle") && !QByteArray(bg->metaObject()->className()).contains("ListItemBackground")) return;   // never style items (buttons)
        QObject *pen = bg->property("border").value<QObject *>();
        if (!pen) return;
        const int a = bg->isVisible() ? int(bg->property("color").value<QColor>().alpha() * bg->opacity()) : 0;
        const int v = lightUi() ? 0 : 255;
        const QColor want(v, v, v, a < 8 ? 0 : (lightUi() ? 56 : 72));          // present on hover and selection, absent when idle
        if (qEnvironmentVariableIsSet("GLASS_DUMP") && a >= 8) fprintf(stderr, "GLASSRIM %s %gx%g a=%d penw=%s pencolor=%s\n", bg->metaObject()->className(), bg->width(), bg->height(), a, qPrintable(pen->property("width").toString()), qPrintable(pen->property("color").value<QColor>().name(QColor::HexArgb)));          // one quiet strength: present on hover and selection, absent when idle
        if (pen->property("color").value<QColor>() != want) { pen->setProperty("width", 1); pen->setProperty("color", want); }
    }
    // alpha < 0: leave theme backgrounds alone (only restyle separators)
    static void restyle(QQuickItem *item, int depth, int alpha)
    {
        if (!item || depth > 40) return;
        const QByteArray cls(item->metaObject()->className());
        if (qEnvironmentVariableIsSet("GLASS_DUMP") && item->width() > 150 && item->height() > 24 && item->height() < 60) fprintf(stderr, "GLASSDUMP %d %s %gx%g vis=%d color=%s hl=%s\n", depth, cls.constData(), item->width(), item->height(), int(item->isVisible()), item->property("color").isValid() ? qPrintable(item->property("color").value<QColor>().name(QColor::HexArgb)) : "-", item->property("highlighted").isValid() ? (item->property("highlighted").toBool() ? "Y" : "n") : "-");
        if (qEnvironmentVariableIsSet("GLASS_DUMP") && cls.startsWith("CategoryItem") && item->property("highlighted").toBool()) {
            std::function<void(QQuickItem *, int)> dump = [&](QQuickItem *it, int d) { fprintf(stderr, "GLASSTREE %*s%s %gx%g vis=%d op=%g color=%s\n", d * 2, "", it->metaObject()->className(), it->width(), it->height(), int(it->isVisible()), it->opacity(), it->property("color").isValid() ? qPrintable(it->property("color").value<QColor>().name(QColor::HexArgb)) : "-"); if (d < 4) for (QQuickItem *c : it->childItems()) dump(c, d + 1); };
            dump(item, 0);
            if (auto *bgi = item->property("background").value<QQuickItem *>()) { fprintf(stderr, "GLASSTREE background:\n"); dump(bgi, 1); }
        }
        if (item->metaObject()->indexOfProperty("highlighted") >= 0 && item->metaObject()->indexOfProperty("background") >= 0) {
            // any delegate-like control. Kirigami's DefaultListItemBackground keeps its outer rectangle transparent and fills an
            // inner one, so rim whichever rectangle actually carries the fill.
            if (auto *bg = item->property("background").value<QQuickItem *>()) {
                refreshRim(bg);
                for (QQuickItem *ch : bg->childItems()) if (ch->property("border").value<QObject *>()) refreshRim(ch);
            }
        }
        if (item->metaObject()->indexOfProperty("highlightItem") >= 0) {      // ListView / GridView: the moving selection tile
            if (auto *hl = item->property("highlightItem").value<QQuickItem *>()) {
                if (hl->property("border").value<QObject *>()) refreshRim(hl);
                else for (QQuickItem *ch : hl->childItems()) if (ch->property("border").value<QObject *>()) { refreshRim(ch); break; }
            }
        }
        const bool thinRect = cls.startsWith("QQuickRectangle") && (item->width() <= 1.01 || item->height() <= 1.01) && item->width() * item->height() > 24;
        if (cls.contains("Separator") || thinRect) {
            // Kirigami draws separators as a text/background mix: a hard grey line. Ours is a faint light hairline, the same
            // language as the glass rim.
            const QColor hair = lightUi() ? QColor(0, 0, 0, 26) : QColor(255, 255, 255, 18);
            if (item->property("color").value<QColor>() != hair) item->setProperty("color", hair);
        }
        // text inputs (the sidebar's search field, fields on pages) keep a nearly solid surface of their own: on a fully clear
        // sidebar their text and placeholder sat straight on the blurred wallpaper and were barely readable in light mode
        if (alpha >= 0 && item->metaObject()->indexOfProperty("placeholderText") >= 0) alpha = qMax(alpha, 218);
        if (alpha >= 0) {
            const auto kids = item->children();
            for (QObject *o : kids) {
                if (!QByteArray(o->metaObject()->className()).contains("Theme")) continue;
                const QVariant v = o->property("backgroundColor");
                if (!v.isValid()) continue;
                const int set = o->property("colorSet").toInt();          // Kirigami.Theme: View 0, Window 1, Button 2, …, Header 6
                if (set != 0 && set != 1 && set != 6) continue;             // surfaces only: buttons, selections, tooltips keep their fill
                // The colour comes from the CURRENT colour scheme, not from the theme object's own value: once we have set a
                // backgroundColor on a Kirigami.Theme it is a custom colour and no longer follows a scheme change, so reading it
                // back kept the dark slab after switching to light (dark page, dark text).
                // (from the live application palette: KColorScheme's cached config still answered with the old scheme here)
                QColor c = qApp->palette().color(QPalette::Active, set == 0 ? QPalette::Base : QPalette::Window);
                if (qEnvironmentVariableIsSet("GLASS_DUMP")) fprintf(stderr, "GLASSBG set=%d had=%s want=%s\n", set, qPrintable(v.value<QColor>().name(QColor::HexArgb)), qPrintable(c.name(QColor::HexArgb)));
                c.setAlpha(alpha);
                if (v.value<QColor>() != c) o->setProperty("backgroundColor", c);
            }
        }
        const auto items = item->childItems();
        for (QQuickItem *ch : items) restyle(ch, depth + 1, alpha);
    }
    void apply()
    {
        if (!m_w || !m_w->window() || !m_w->isVisible()) return;
        const bool sidebar = m_w->width() <= 480 && m_w->width() * 2 < m_w->window()->width();
        const bool frostedWindow = sidebar || m_w->window()->property("_glass_frosted_window").toBool();
        if (!frostedWindow) { restyle(m_w->rootObject(), 0, -1); return; }
        if (!m_done) {
            m_done = true;
            m_w->setAttribute(Qt::WA_AlwaysStackOnTop);        // needed for a see-through QQuickWidget
            m_w->setAttribute(Qt::WA_TranslucentBackground);
            m_w->setClearColor(Qt::transparent);
            m_w->window()->setProperty("_glass_frosted_window", true);
            if (QWindow *h = m_w->window()->windowHandle()) KWindowEffects::enableBlurBehind(h, true);   // frost, not just see-through
            m_w->window()->update();
        }
        // Sidebars: fully clear, the window's frosted fill IS their surface. Content: a whisper of the same glass, so the
        // page belongs to the window instead of sitting on it as a black slab. The window fill is already `chrome` opaque,
        // so the layer on top only needs to make up the difference to the wanted effective opacity.
        const qreal chrome = Darkly::StyleConfigData::toolBarOpacity() / 100.0, want = Darkly::StyleConfigData::glassContentOpacity() / 100.0;
        const qreal layer = chrome >= 1.0 ? 1.0 : qBound(0.0, 1.0 - (1.0 - want) / (1.0 - chrome), 1.0);
        // A page is not ONE surface: the KCM's window-coloured wrapper and the page's view-coloured background lie on top of each
        // other, and each gets this alpha. Two stacked layers of a give 1-(1-a)^2, so each one gets the square-root share;
        // without this the page came out ~90 % opaque and read as solid next to the clear sidebar.
        // The content's glass is painted by the WINDOW, under the QQuickWidget (Paint branch of eventFilter, property
        // "_glass_content_alpha"), and the QML backgrounds are fully clear, like the sidebar's. A translucent colour INSIDE the
        // QQuickWidget is composited wrongly (its premultiplied picture is blended as if it were not: colour x alpha twice).
        // Dark on dark that went unnoticed; in light mode the white page came out as a flat grey slab.
        m_w->setProperty("_glass_content_alpha", sidebar ? 0.0 : layer);
        restyle(m_w->rootObject(), 0, 0);
        if (!sidebar) m_w->window()->update();
    }
    QPointer<QQuickWidget> m_w;
    bool m_done = false;
};
}
#endif
...
void Style::polish(QWidget *widget)
{
    ...
    if (auto glassQuick = qobject_cast<QQuickWidget *>(widget)) {
        if (!_isOpaque && !glassQuick->property("_glass_sidebar_helper").toBool()) { glassQuick->setProperty("_glass_sidebar_helper", true); new GlassQuickSidebar(glassQuick); }
    }
        ...
        if (widget && event->type() == QEvent::Paint) {
            if (widget->isWindow() && !_isKonsole && widget->testAttribute(Qt::WA_StyledBackground) && widget->testAttribute(Qt::WA_TranslucentBackground)) {
                ...
                case Qt::Sheet: {
                    ...
                    {
                        QColor fill(widget->palette().color(QPalette::Window));
                        if (widget->property("_glass_frosted_window").toBool()) {   // Glass: the whole window fill is frost; solid QML/widget content covers what should be solid
                            fill.setAlphaF(StyleConfigData::toolBarOpacity() / 100.0);
                            p.setCompositionMode(QPainter::CompositionMode_Source);
                        }
                        p.fillRect(widget->rect(), fill);
                        p.setCompositionMode(QPainter::CompositionMode_SourceOver);
                        // Glass: the content glass of QML pages (see GlassQuickSidebar::apply): the view colour over the frost
                        if (widget->property("_glass_frosted_window").toBool()) {
                            const auto quicks = widget->findChildren<QQuickWidget *>();
                            for (QQuickWidget *qw : quicks) {
                                const qreal a = qw->property("_glass_content_alpha").toReal();
                                if (a <= 0.0 || !qw->isVisibleTo(widget)) continue;
                                QColor view(qApp->palette().color(QPalette::Active, QPalette::Base)); view.setAlphaF(a);
                                p.fillRect(QRect(qw->mapTo(widget, QPoint(0, 0)), qw->size()), view);
                            }
                        }
                    }
...
bool Style::drawMenuBarItemControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    const bool sunken(enabled && (state & State_Sunken));
    const bool useStrongFocus(false);   // Glass: same glass tile as menu rows, text keeps its normal colour
    ...
    if (selected || sunken) {
        ...
        const QRectF tile = QRectF(rect).adjusted(1, 2, -1, -2);
        painter->setPen(Qt::NoPen);
        painter->setBrush(QColor(255, 255, 255, sunken ? 56 : 32));
        painter->drawRoundedRect(tile, radius, radius);
        QLinearGradient rim(0, tile.top(), 0, tile.bottom());
    ...
    */
    if (!menuItemOption->icon.isNull()) {
        ...
        // render outline
        if (false && !useStrongFocus && (selected || sunken)) {   // Glass: no underline
            QColor outlineColor;
            if (sunken)
...
bool Style::drawMenuItemControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    const bool reverseLayout(option->direction == Qt::RightToLeft);
    // Glass: the hovered row is a small pane of glass (translucent fill + lit rim) and the text keeps its normal colour.
    // Darkly's "strong focus" painted an opaque slab in the focus colour with highlighted text on it — with our light,
    // translucent selection colour that became a bright bar with unreadable text.
    const bool useStrongFocus(false);
    ...
    if (selected || sunken) {
        painter->save();
        painter->setRenderHints(QPainter::Antialiasing);
        const qreal radius = qMax<qreal>(2.0, _helper->frameRadius(0, -1));
        const QRectF tile = QRectF(rect).adjusted(2, 1, -2, -1);
        painter->setPen(Qt::NoPen);
        painter->setBrush(QColor(255, 255, 255, sunken ? 56 : 36));
        painter->drawRoundedRect(tile, radius, radius);
        QLinearGradient rim(0, tile.top(), 0, tile.bottom());
        rim.setColorAt(0.0, QColor(255, 255, 255, 60));
        rim.setColorAt(1.0, QColor(255, 255, 255, 22));
    ...
    if (!menuItemOption->text.isEmpty()) {
        ...
        // render hover and focus
        if (false && !useStrongFocus && (selected || sunken)) {   // Glass: no underline, the tile is the feedback
            QColor outlineColor;
            if (sunken)
                outlineColor = _helper->focusColor(palette);
            else if (selected)
