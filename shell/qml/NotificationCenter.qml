...
// Clear. Opening it marks everything as read (the bell's dot goes away).
import QtQuick
...
Item {
    ...
    implicitWidth: 380
    implicitHeight: 52 + (list.count > 0 ? Math.min(list.contentHeight, 520) + 6 : 84)
    onOpenChanged: if (open) { history.lastRead = undefined; list.positionViewAtBeginning() }
    function toggleDnd() {
    ...
    property int tick: 0
    // ---- grouping: entries per app, which row starts an app's run, which apps are opened up
    property var counts: ({})
    ...
    function recount() { const c = {}, f = {}; for (let i = 0; i < history.count; ++i) { const a = appOf(i); if (c[a] === undefined) { c[a] = 0; f[a] = i } c[a]++ } counts = c; firstRow = f }
    ...
    Item { x: 8; y: 6; width: parent.width - 16; height: 40
        ...
        Row { anchors.right: parent.right; anchors.verticalCenter: parent.verticalCenter; spacing: 6
            ...
            HeadBtn { label: "Clear"; icon: "edit-clear-history-symbolic"; visible: list.count > 0; onTapped: history.clear(NotificationManager.Notifications.ClearExpired) } } }

    ListView { id: list; x: 8; y: 52; width: parent.width - 16; height: Math.min(contentHeight, 520); clip: true; spacing: 0; model: history
        ...
        delegate: Item { id: row
            ...
            readonly property bool hiddenRow: groupSize > 1 && !open && !first
