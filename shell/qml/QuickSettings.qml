...
// tiles (network, Bluetooth, do not disturb, night light, displays, settings), and brightness / volume as thin glass
// sliders. Two detail pages slide in: Sound (devices) and Network (connections). Same visual language as the launcher,
...
Item {
    ...
    readonly property var themeNames: { const hue = n => { const h = Qt.color((Config.themes[n] || {}).accent || "#888").hslHue; return h > 0.93 ? h - 1 : h }   // literal-ok: fallback swatch
        return Object.keys(Config.themes).sort((a, b) => hue(a) - hue(b)) }       // rainbow order (the config's keys arrive alphabetically)
    ...
    Control.NightLight { id: night }
    NetParts.Network { id: net }
    readonly property QtObject bt: BluezQt.Manager
    ScreenBrightnessControl { id: brightness; isSilent: true }     // no OSD per step while dragging our own slider
    Connections { id: screens; target: brightness.displays
        property var info: []
        function update() {
            const roles = ["label", "brightness", "maxBrightness", "displayName"].map(r => target.KItemModels.KRoleNames.role(r));
            info = [...Array(target.rowCount()).keys()].map(i => { const x = target.index(i, 0);
    ...
    readonly property var screen0: screens.info.length > 0 ? screens.info[0] : null
    readonly property bool canDim: brightness.isBrightnessAvailable && screen0 !== null
    readonly property var sink: PreferredDevice.sink
    readonly property bool hasSink: sink && sink.name !== "auto_null"
    readonly property int volumePct: hasSink ? Math.round(sink.volume / PulseAudio.NormalVolume * 100) : 0
    function volIcon() { if (!hasSink || sink.muted || volumePct === 0) return "audio-volume-muted-symbolic"; return volumePct < 34 ? "audio-volume-low-symbolic" : volumePct < 67 ? "audio-volume-medium-symbolic" : "audio-volume-high-symbolic" }
    ...
    readonly property string netName: { const t = net.netStatusText; if (!t) return "";
        return String(t).split("\n").map(l => { const m = l.match(/Connected to (.*)$/); return m ? m[1] : l }).filter(x => x !== "").join(" · ") }
    readonly property bool btOn: bt.bluetoothOperational
    readonly property string btName: { for (let i = 0; i < bt.devices.length; ++i) if (bt.devices[i].connected) return bt.devices[i].name; return "" }
    ...
    function toggleBt() { const on = !bt.bluetoothOperational; bt.bluetoothBlocked = !on; for (let i = 0; i < bt.adapters.length; ++i) bt.adapters[i].powered = on }
    function toggleDnd() {
    ...
    Column { id: mainPage; x: qs.pad; y: qs.pad; width: qs.width - 2 * qs.pad; spacing: qs.gap + 2
        ...
        Item { width: parent.width; height: 40
            ...
            Row { id: headBtns; anchors.right: parent.right; anchors.verticalCenter: parent.verticalCenter; spacing: 6
                ...
                RoundBtn { icon: "system-lock-screen-symbolic"; onTapped: { session.lock(); qs.closeRequested() } }
        ...
        Grid { enabled: !qs.paletteOpen   /* pointer handlers keep a passive grab: the catcher above cannot stop them, so a swatch click also hit the tile under it */; width: parent.width; columns: 2; spacing: qs.gap
            ...
            Tile { width: parent.tw; icon: net.activeConnectionIcon || "network-wired-symbolic"; title: "Network"; status: qs.netName !== "" ? qs.netName : "Not connected"; on: qs.netName !== ""; more: true
                onToggled: qs.wifiThere ? net.handler.enableWireless(!qs.wifiOn) : qs.page = "network"; onOpened: qs.page = "network" }
            Tile { width: parent.tw; icon: qs.btOn ? "network-bluetooth-activated-symbolic" : "network-bluetooth-inactive-symbolic"; title: "Bluetooth"
                status: !qs.btOn ? "Off" : (qs.btName !== "" ? qs.btName : "On"); on: qs.btOn; more: true; onToggled: qs.toggleBt(); onOpened: qs.openKcm("kcm_bluetooth") }
            ...
            Tile { width: parent.tw; icon: "redshift-status-on"; title: "Night Light"; status: night.statusText; on: night.on; more: true; onToggled: night.toggle(); onOpened: qs.openKcm("kcm_nightlight") }
            Tile { width: parent.tw; icon: "video-display-symbolic"; title: "Displays"; status: qs.screen0 ? String(qs.screen0.label || "") : ""; onToggled: qs.openKcm("kcm_kscreen") }
        ...
        Rectangle { enabled: !qs.paletteOpen; width: parent.width; height: sliders.implicitHeight + 8; radius: 18; color: Config.fg(0.055); border.width: 1; border.color: Config.fg(0.09)
            Column { id: sliders; y: 4; width: parent.width
                SliderRow { width: parent.width; visible: qs.canDim; icon: "brightness-high-symbolic"
                    value: qs.screen0 ? 100 * qs.screen0.brightness / Math.max(1, qs.screen0.maxBrightness) : 0
                    onMoved: v => { if (qs.screen0) brightness.setBrightness(qs.screen0.displayName, Math.round(v / 100 * qs.screen0.maxBrightness)) } }
                ...
                SliderRow { width: parent.width; icon: qs.volIcon(); value: qs.volumePct; more: true; enabled_: qs.hasSink
                    onMoved: v => { if (qs.hasSink) { qs.sink.volume = Math.round(v) * PulseAudio.NormalVolume / 100; if (qs.sink.muted && v > 0) qs.sink.muted = false; qs.osdRequested(qs.volIcon(), Math.round(v) / 100) } }
                    onIconTapped: if (qs.hasSink) qs.sink.muted = !qs.sink.muted
    ...
    Column { id: soundPage; x: qs.pad; y: qs.pad; width: qs.width - 2 * qs.pad; spacing: qs.gap
        ...
        Rectangle { width: parent.width; height: 54; radius: 18; color: Config.fg(0.055); border.width: 1; border.color: Config.fg(0.09)
            SliderRow { width: parent.width; anchors.verticalCenter: parent.verticalCenter; icon: qs.volIcon(); value: qs.volumePct; enabled_: qs.hasSink
                onMoved: v => { if (qs.hasSink) { qs.sink.volume = Math.round(v) * PulseAudio.NormalVolume / 100; qs.osdRequested(qs.volIcon(), Math.round(v) / 100) } }
                onIconTapped: if (qs.hasSink) qs.sink.muted = !qs.sink.muted } }
        Label_ { text: "Output" }
        Column { width: parent.width
            Repeater { model: PulseObjectFilterModel { filterOutInactiveDevices: true; sourceModel: SinkModel {} }
        ...
        Label_ { text: "Input" }
        Column { width: parent.width
            Repeater { model: PulseObjectFilterModel { filterOutInactiveDevices: true; sourceModel: SourceModel {} }
    ...
    Column { id: netPage; x: qs.pad; y: qs.pad; width: qs.width - 2 * qs.pad; spacing: qs.gap
        ...
        Item { width: parent.width; height: 44; visible: qs.wifiThere
            ...
            GlassSwitch { anchors.right: parent.right; anchors.rightMargin: 6; anchors.verticalCenter: parent.verticalCenter; checked: qs.wifiOn; onToggled: on => net.handler.enableWireless(on) } }
        ListView { width: parent.width; height: Math.min(contentHeight, 6 * 42); clip: true; interactive: contentHeight > height; boundsBehavior: Flickable.StopAtBounds
            model: net.appletProxyModel
            delegate: ListRow { required property var model; width: ListView.view.width
                readonly property bool active: model.ConnectionState === PlasmaNM.Enums.Activated
                icon: model.ConnectionIcon || "network-wired-symbolic"; title: model.ItemUniqueName || model.Name || ""; current: active
                note: active ? "Connected" : (model.ConnectionState === PlasmaNM.Enums.Activating ? "Connecting…" : (model.SecurityType > 0 && !model.ConnectionPath ? "Secured" : ""))
                onTapped: { if (active) net.handler.deactivateConnection(model.ConnectionPath, model.DevicePath);
                            else if (model.ConnectionPath) net.handler.activateConnection(model.ConnectionPath, model.DevicePath, model.SpecificPath);
