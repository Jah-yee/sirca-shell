...
cmake --install build
# a user-prefix install puts the units where systemd --user looks for them
if [ "$PREFIX" = "$HOME/.local" ]; then mkdir -p "$HOME/.config/systemd/user"; for u in sirca-shell.service sirca-shell-fallback.service; do
    [ -f "$PREFIX/lib/systemd/user/$u" ] && ln -sf "$PREFIX/lib/systemd/user/$u" "$HOME/.config/systemd/user/$u"; done; fi
