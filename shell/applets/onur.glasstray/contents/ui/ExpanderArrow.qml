...
PlasmaCore.ToolTipArea {
    ...
    Kirigami.Icon {
        ...
        source: {
            if (Plasmoid.location === PlasmaCore.Types.TopEdge || Plasmoid.location === PlasmaCore.Types.Floating) { // hosted glass bar is Floating
                return "arrow-down";
    ...
    // --- onur: glow instead of the frosted active tile while the hidden-items popup is open ---
    readonly property real glowOpacity: (systemTrayState.expanded && !systemTrayState.activeApplet) ? 1 : 0
    MultiEffect {
        z: -1
        anchors.fill: parent
        source: expandedArrow
        autoPaddingEnabled: true
        blurEnabled: true; blur: 1.0; blurMax: 28
        brightness: 0.5; saturation: -0.2
        scale: 1.6; transformOrigin: Item.Center
        opacity: tooltip.glowOpacity * 0.75
    ...
    MultiEffect {
        ...
        source: expandedArrow
        autoPaddingEnabled: true
        blurEnabled: true; blur: 0.8; blurMax: 10
        brightness: 0.5
        scale: 1.15; transformOrigin: Item.Center
        opacity: tooltip.glowOpacity * 0.6
        Behavior on opacity { NumberAnimation { duration: Kirigami.Units.longDuration; easing.type: Easing.OutCubic } }
    }
    // --- end onur ---
