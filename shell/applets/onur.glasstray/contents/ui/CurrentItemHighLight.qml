...
KSvg.FrameSvgItem {
    ...
    function updateHighlightedItem() {
        if (systemTrayState.expanded) {
            ...
            } else { // 'Show hidden items' popup: onur — no tile, the expander arrow glows instead (ExpanderArrow.qml)
                highlightedItem = null;
            }
        } else {
            highlightedItem = null;
        }
        currentItemHighLight.opacity = (systemTrayState.expanded && highlightedItem) ? 1 : 0
    }
