...
QtObject {
    ...
    // Sirca Shell: cards are laid out by the Column they are parented into
    property Item cardsHost: null
    readonly property int cardCount: popupInstantiator.count
    function positionPopups() { return; }
    function positionPopupsStock() {
        if (!plasmoid) {
            return;
        }
        ...
        delegate: GlassCard {
            ...
            modelInterface {
                ...
                onForceActiveFocusRequested: {
                    // NOTE this is our "plasmoid" property from above, don't port this to Plasmoid attached property!
                    /* cards live in the bar surface, which takes focus on click */
                }
