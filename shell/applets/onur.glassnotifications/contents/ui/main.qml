...
PlasmoidItem {
    ...
    // ---- Sirca Shell hooks: the shell adopts `glassCardsItem` into a lobe of its top bar. Popups are Items parented
    // into it (GlassCard.qml) instead of windows; everything else (history, do-not-disturb, jobs) is the stock applet.
    readonly property Item glassCardsItem: cardsColumn
    readonly property int glassCardCount: GlassGlobals.cardCount
    Column { id: cardsColumn; visible: false; spacing: 8; width: GlassGlobals.popupWidth + 20
        move: Transition { NumberAnimation { properties: "y"; duration: 180; easing.type: Easing.OutCubic } }
        Component.onCompleted: {
            GlassGlobals.cardsHost = cardsColumn;
            // The stock singleton gets created as soon as anything reads Globals.inhibited, and with no plasmoid adopted its
            // model limit is 0 = unlimited: it would pop every notification a second time as a window at 0,0. Switch it off.
            Notifications.Globals.popupInstantiator.active = false;
        } }
    ...
    toolTipSubText: {
        ...
        if (!NotificationManager.Server.valid) {
            ...
            var actualUnread = historyModel.unreadNotificationsCount - GlassGlobals.popupNotificationsModel.activeNotificationsCount;
            if (actualUnread > 0) {
    ...
    onExpandedChanged: expanded => {
        if (expanded) {
            GlassGlobals.popupNotificationsModel.hideInhibitionSummary();
        } else {
    ...
    compactRepresentation: Notifications.CompactRepresentation {
        activeCount: GlassGlobals.popupNotificationsModel.activeNotificationsCount
    ...
    fullRepresentation: Notifications.FullRepresentation {
    ...
    Component.onDestruction: {
        GlassGlobals.forget(root);
    }

    Connections {
        target: GlassGlobals.popupNotificationsModel
