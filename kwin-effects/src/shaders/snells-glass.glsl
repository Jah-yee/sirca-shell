...
GlassFragment snellsRefraction(vec2 position, vec2 halfBlurSize, vec4 cornerRadius, float minHalfSize, float dist, float edgeFactor, float concaveFactor)
{
    ...
    float dxp = shapeDist(position + vec2(eps, 0.0), halfBlurSize, cornerRadius);
    float dxn = shapeDist(position - vec2(eps, 0.0), halfBlurSize, cornerRadius);
    float dyp = shapeDist(position + vec2(0.0, eps), halfBlurSize, cornerRadius);
    float dyn = shapeDist(position - vec2(0.0, eps), halfBlurSize, cornerRadius);
